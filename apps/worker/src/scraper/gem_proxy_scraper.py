...
def _call_data_endpoint(keyword: str, sort: str, page: int, cookies: dict, csrf: str) -> dict:
    payload = {
        "page": page,
        "param": {"searchBid": keyword, "searchType": "contains"},
        "filter": {
...
def scrape_gem_direct(
    ...
    """
    Fetch one page (10 bids) from GeM in GeM's exact order.
    Returns {"bids": [...], "total": N, "page": N, ...}
    """
    ...
    for i, doc in enumerate(docs):
        ...
        bid_id     = doc.get("id", "")   # numeric Solr ID → /bidlisting/{id}
    ...
    return {"bids": bids, "total": total, "page": page, "keyword": keyword, "sort": sort}
